...
H_bestModDenSup=cell(5,realizations);
...
k1=4; %number of communities per layer
k2=3;
k3=7;
k4=5;
k5=3;
kc=7; %number of common communities
kp1=0;
kp2=0;
kp3=0;
kp4=0;
...
%         S1new=S1new/norm(diag(S1new));
        ...
        S2new=S2.*((H'*A2*H + a*(S1+S3+S4+S5) + b*(1/trace(S2))*eye(kc))./(H'*H*S2*(H'*H) + H'*H2*H2'*H + c*S2)).^eta;
%         S2new=S2new/norm(S2new);
%         S2new=S2new/norm(diag(S2new));
        if all(isnan(S2new),'all')==1
        S2new=diag(rand(kc,1)); 
        end 
        S3new=S3.*((H'*A3*H + a*(S2+S1+S4+S5) + b*(1/trace(S3))*eye(kc))./(H'*H*S3*(H'*H) + H'*H3*H3'*H + c*S3)).^eta;
%         S3new=S3new/norm(S3new);
%         S3new=S3new/norm(diag(S3new));
        if all(isnan(S3new),'all')==1
        S3new=diag(rand(kc,1));
        end 
        S4new=S4.*((H'*A4*H + a*(S2+S1+S3+S5) + b*(1/trace(S4))*eye(kc))./(H'*H*S4*(H'*H) + H'*H4*H4'*H + c*S4)).^eta;
        if all(isnan(S4new),'all')==1
        S4new=diag(rand(kc,1));
        end 
        S5new=S5.*((H'*A5*H + a*(S2+S1+S3+S4) + b*(1/trace(S5))*eye(kc))./(H'*H*S5*(H'*H) + H'*H5*H5'*H + c*S5)).^eta;
...
for g=1:kc
    for nodes=1:n
    if all(H(nodes,g)>H1(nodes,:)) && all(H(nodes,g)>H2(nodes,:)) && all(H(nodes,g)>H3(nodes,:)) && all(H(nodes,g)>H4(nodes,:)) && all(H(nodes,g)>H5(nodes,:))
        I1(nodes)=g;
        I2(nodes)=g;
        I3(nodes)=g;
        I4(nodes)=g;
        ...
        I2(nodes)= i2(nodes)+kc;
        nodes=find(I3==0);        
        [~,i3(nodes)] = max(H3(nodes,:),[],2);
        I3(nodes)= i3(nodes)+kc;
        nodes=find(I4==0);
        [~,i4(nodes)] = max(H4(nodes,:),[],2);
        I4(nodes)= i4(nodes)+kc;
        nodes=find(I5==0);
        [~,i5(nodes)] = max(H5(nodes,:),[],2);
        I5(nodes)= i5(nodes)+kc;
...
[ModDenjSup,ModDenNormjSup]=ModDen(kp1+kp2+kp3+kp4+kp5+kc,[A1 zeros(n,4*n);zeros(n) A2 zeros(n,3*n);zeros(n,2*n) A3 zeros(n,2*n);zeros(n,3*n) A4 zeros(n); zeros(n,4*n) A5],ClustersSupra);
